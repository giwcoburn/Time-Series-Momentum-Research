...
The logic of the engine moves beyond the typical "buy and hold" startegy. Multi-frequency signal bleeding allows for dynamic adjustment of exposure to assets based on trend and volatility.
Instead of a 12-month "lookback" signal, the algorithm blends three time horizons together to create a contonuous "conviction score".
...
The basic description of the mechanism is that leverage is reduced during amrket chaos, and increased in calm periods. 
...
The chart below compares the Optimsied Blended Stretegy (GreeN) against the Basic Model (Blue) and the S&P500 benchmark (Grey).
...
During 2022, traditional portfolios failed due to stock/bond correlation spiking to 1.0. However this strategy sucessfuly identifies the downtrend in equities and uptrend in commodities/USD, generating positive retruns when liquidity was needed most.

4. Statistical variation:
To ensure results were not random or luck, an Ordianry Least Squares regressio was conducted against the S&P500.
--- FINAL STATS ---
Annualized Alpha: 11.50% - Pure excess return after risk adjustment
Beta to S&P 500:  -0.08 - Stretegy is market neutral, but slightly inverse
...
A: Clone repository Bash
git clone https://github.com/yourusername/Time-Series-Momentum.git

B: Install dependencies bash
pip install yfinance pandas numpy matplotlib statsmodels

C: Run the engine bash
